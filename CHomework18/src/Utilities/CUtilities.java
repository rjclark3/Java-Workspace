...
public class CUtilities
{
	...
	// ----------------------------------------------------------------------
	// Name: IsValidZipCode
	// Abstract: Use regular expressions to verify that the zip code is in
	//			a valid format.
	// ----------------------------------------------------------------------
	public static boolean IsValidZipCode( String strZipCode )
	{
		...
		try
		{
			...
			// #####-#### 	5-4 digits
			String strPattern2 = chrStart + "\\d{5}\\-\\d{4}" + chrStop;
	...
	//			 date format.
	// ----------------------------------------------------------------------
	public static Boolean IsValidDate( String strDate, String strPattern )
	{
		boolean blnIsValidDate = false;
		...
		{		
			// Compare
			blnIsValidDate = strDate.matches( strPattern );
		}
		catch( Exception excError )
		{
			// Display Error Message
	...
	public static Boolean IsValidEmailAddress( String strEmailAddress )
	{
		...
		{	
			...
			String strComplexPattern = chrStart 
							 + "[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+"
							 + "(" + strTopLevelDomainList + chrOR + strCountryCodes + ")"
							 + chrStop;

			// Make lowercase so it's easier to match complex pattern.
			...
			String strEasyPattern = chrStart
								  + "[a-zA-Z][a-zA-Z0-9\\.\\-]*"
								  + "@"
			...
			// Does it match?
			if( strEmailAddress.matches( strEasyPattern ) == true )
			{
				// Yes
